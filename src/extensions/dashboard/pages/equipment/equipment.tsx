...
import { getCurrentPlan, hasFeature, planLabels, requiredPlan } from '../../../../lib/plans';
...
const EquipmentPage: FC = () => {
  ...
  const editAsset = (asset: Asset) => {
    ...
    setForm({
      ...
      dailyRate: rateToInput(asset.dailyRateCents),
      weeklyRate: rateToInput(asset.weeklyRateCents),
      monthlyRate: rateToInput(asset.monthlyRateCents),
      discountAfterDays: asset.discountAfterDays ? String(asset.discountAfterDays) : '',
      discountPercent: asset.discountPercent ? String(asset.discountPercent).replace('.', ',') : '',
  ...
  const deactivate = async (asset: Asset) => {
    ...
    try {
      const updated = await items.update(COLLECTION, {
        ...
        discountAfterDays: asset.discountAfterDays ?? 0,
        discountPercent: asset.discountPercent ?? 0,
        currency: asset.currency ?? 'CAD',
        serialNumber: asset.serialNumber,
        image: asset.image,
        notes: asset.notes,
                    ...
                    <tbody>
                      {filteredAssets.map((asset, index) => (
                        <tr key={asset._id ?? `${asset.assetNumber}-${index}`} style={{ borderTop: '1px solid #e5e7eb' }}>
                          ...
                          <td style={{ textAlign: 'right' }}>{money(asset.dailyRateCents, asset.currency)}</td>
                          <td style={{ textAlign: 'right' }}>{money(asset.weeklyRateCents, asset.currency)}</td>
...
const PremiumField: FC<{ enabled: boolean; feature: Parameters<typeof requiredPlan>[0]; children: ReactNode }> = ({ enabled, feature, children }) => (
  <div>
    {children}
    {!enabled && (
