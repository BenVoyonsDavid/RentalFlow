...
class RentalFlowBookingElement extends HTMLElement {
  ...
  private getInput(name: string): string {
    const input = this.root.querySelector<HTMLInputElement | HTMLTextAreaElement>(`[name="${name}"]`);
  ...
  private async submitBooking() {
    ...
    const customer = {
      name: this.getInput('customerName'),
      email: this.getInput('customerEmail'),
      phone: this.getInput('customerPhone'),
      addressLine1: this.getInput('addressLine1'),
      addressLine2: this.getInput('addressLine2'),
      city: this.getInput('city'),
      region: this.getInput('region'),
      postalCode: this.getInput('postalCode'),
      country: this.getInput('country') || 'Canada',
    };
    ...
    this.root.innerHTML = `
      <style>
        ...
        *{box-sizing:border-box}.wrap{max-width:1100px;margin:auto;background:#fff;border:1px solid var(--border);border-radius:18px;overflow:hidden;box-shadow:0 12px 35px rgba(30,55,90,.08)}
        ...
        .assets{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:11px}.asset{width:100%;display:grid;grid-template-columns:28px 1fr auto;gap:10px;align-items:center;text-align:left;padding:14px;border:1px solid var(--border);border-radius:12px;background:#fff;color:inherit;cursor:pointer}.asset:hover:not(:disabled){border-color:#9bbdfd;background:#fbfdff}.asset.selected{border:2px solid var(--rf);background:var(--rf-soft)}.asset.unavailable{opacity:.5}.asset-main{display:flex;flex-direction:column;gap:3px}.asset-main small{color:var(--muted)}.asset-price{font-weight:800;white-space:nowrap}.check{width:22px;height:22px;border:2px solid #b9c5d6;border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;background:${this.selected.size ? 'transparent' : 'transparent'}}.asset.selected .check{background:var(--rf);border-color:var(--rf)}
          ...
          ${this.loading ? '<div class="loading">Chargement de la disponibilité…</div>' : this.result ? `
            ...
            ${selectedAssets.length ? `
              <section class="section">
                ...
                <div class="grid">
                  <div class="field"><label>Nom complet *</label><input name="customerName" autocomplete="name"></div>
                  <div class="field"><label>Courriel *</label><input name="customerEmail" type="email" autocomplete="email"></div>
                  <div class="field"><label>Téléphone ${requiredPhone ? '*' : ''}</label><input name="customerPhone" autocomplete="tel"></div>
                  <div class="field"><label>Adresse ${requiredAddress ? '*' : ''}</label><input name="addressLine1" autocomplete="street-address"></div>
                  <div class="field"><label>Adresse 2</label><input name="addressLine2"></div>
                  <div class="field"><label>Ville ${requiredAddress ? '*' : ''}</label><input name="city" autocomplete="address-level2"></div>
                  <div class="field"><label>Province / État ${requiredAddress ? '*' : ''}</label><input name="region" autocomplete="address-level1" value="QC"></div>
                  <div class="field"><label>Code postal ${requiredAddress ? '*' : ''}</label><input name="postalCode" autocomplete="postal-code"></div>
                  <div class="field"><label>Pays ${requiredAddress ? '*' : ''}</label><input name="country" autocomplete="country-name" value="Canada"></div>
                  <div class="field full"><label>Notes</label><textarea name="notes" placeholder="Information utile concernant votre réservation"></textarea></div>
                </div>
              </section>
              <section class="section">
